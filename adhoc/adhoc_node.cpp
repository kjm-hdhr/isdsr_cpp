    ...
    for(auto p = addrs; p!=nullptr; p=p->ifa_next){
        ...
        if(this->if_name==s){
            if(p->ifa_addr->sa_family==AF_INET){
                //uint32_t ip_addr=((struct sockaddr_in*)p->ifa_addr)->sin_addr.s_addr;
...
void adhoc_node::send_msg(array<uint8_t,ADDR_SIZE> &next,vector<uint8_t> &buf){
    ...
    std::cerr<<"]"<<std::endl;
    uint32_t next_ip=adhoc_util::array_to_ip(next);
...
void adhoc_node::data_send(ar_packet* pkt){
    array<uint8_t,ADDR_SIZE> *next=pkt->get_next();
    uint32_t next_ip=adhoc_util::array_to_ip(*next);
    vector<uint8_t> buf;
...
void adhoc_node::establish_route(array<uint8_t,ADDR_SIZE> &dest){
    vector<uint8_t> buf;
    array<uint8_t,ADDR_SIZE>* next;
