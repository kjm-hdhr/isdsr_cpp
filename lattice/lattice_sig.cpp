...
void lattice_sig::key_derivation(){
    ...
    this->signagg.crypto_sign_keypair_skid(pk_agg, skid,own_id,msk);
}
void lattice_sig::sign(isdsr_packet &p){
    ...
    this->signagg.crypto_sign_signatre_skid(
    ...
    this->signagg.crypto_sign_signature_agg(agg_sig,prev_aggsig,sig,p.get_ri_length()-1);
...
bool lattice_sig::verify(isdsr_packet &p){
    ...
    int v=this->signagg.crypto_sign_verify_agg(pk_agg,sig,ri_bytes,p.get_ri_length());
	return v;
}