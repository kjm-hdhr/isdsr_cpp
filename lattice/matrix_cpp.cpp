...
void matrix_operation::print_mat33(int32_t mat33[9]){
	int i;
	printf("mat33=[\n");
	for(i=0;i<3;i++){
		printf("%d,",mat33[i*3]);
...
void matrix_operation::print_mat44(int32_t mat44[16]){
	int i;
	printf("mat44=[\n");
	for(i=0;i<4;i++){
		printf("%d,",mat44[i*4]);
...
uint64_t matrix_operation::invmod (uint64_t a, uint64_t p)
{
...
void matrix_operation::put_value33(int32_t mat33[9], int r, int c, int32_t v){
	mat33[(r-1)*3+(c-1)]=v;
}
int32_t matrix_operation::get_value33(int32_t mat33[9], int r, int c){
	return mat33[(r-1)*3+(c-1)];
}
void matrix_operation::put_value44(int32_t mat44[16], int r, int c, int32_t v){
	mat44[(r-1)*4+(c-1)]=v;
}
int32_t matrix_operation::get_value44(int32_t mat44[16], int r, int c){
	return mat44[(r-1)*4+(c-1)];
}

int32_t matrix_operation::mat33_det_elem(
...
int32_t matrix_operation::mat33_det(int32_t mat33[9], int32_t p){
	int64_t det=0;
	// r1c1*r2c2*r3c3
...
void matrix_operation::make_cofactor_matrix(int32_t mat44[16], int32_t mat33[9], int r, int c){
	// (r-1)*4 ~ (r-1)*4+4
	// 
	int i,j;
	int index;
...
int32_t matrix_operation::make_adjugate_matrix_elem(int r, int c, int32_t mat44[16], int p){
	int32_t mat33[9];
...
void matrix_operation::make_adjugate_mat44(int32_t a[16], int32_t mat44[16],int p){
...
int32_t matrix_operation::mat44_det(int32_t mat44[16], int32_t p){
	int32_t mat33[9];
	int64_t a;
	int64_t ret;
...
int32_t matrix_operation::multiply(int32_t a, int32_t b, int p){
	int64_t ret=a;
	ret=(ret*b)%p;
	return ret;
}
void matrix_operation::multipy_elem(int row, int col, int32_t a[16], int32_t b[16], int32_t c[16], int p){
	//C=A*B
	c[(row-1)*4+(col-1)]=
...
void matrix_operation::multiply_mat(int32_t a[16], int32_t b[16], int32_t c[16], int p){
//C=A*B
// mat44[16]={11,12,13,14,21,22,23,24,31,32,33,34,41,42,43,44}
// (r-1)*4+(c-1)
// r1c1= 0 r1c2= 1 r1c3= 2 r1c4= 3
